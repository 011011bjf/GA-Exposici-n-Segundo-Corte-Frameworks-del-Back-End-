...
\begin{frame}{Flujo general}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={draw, rounded corners, minimum width=3.1cm, minimum height=1.2cm, align=center, font=\small}
    ]
        ...
        \draw[-{Stealth}] ([yshift=3pt]cliente.east) -- node[above, font=\tiny]{JSON / HTTP} ([yshift=3pt]backend.west);
        \draw[-{Stealth}] ([yshift=-3pt]backend.west) -- node[below, font=\tiny]{respuesta} ([yshift=-3pt]cliente.east);

        \draw[-{Stealth}] ([yshift=3pt]backend.east) -- ([yshift=3pt]bd.west);
        \draw[-{Stealth}] ([yshift=-3pt]bd.west) -- ([yshift=-3pt]backend.east);
    \end{tikzpicture}%
    }
\end{frame}
...
\begin{frame}{Cierre}
    \begin{itemize}
        ...
        \item Prueba end-to-end contra el backend real: pendiente por configuración de entorno.
        \item Diseño de esta presentación: texto mínimo y apoyo visual \cite{alley2013, mayer2009}.
    \end{itemize}
    \vspace{10pt}
    \begin{center}
